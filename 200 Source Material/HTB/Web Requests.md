...
Resources over HTTP are accessed via a `URL`, which offers many more specifications than simply specifying a website we want to visit. Let's look at the structure of a URL: ![[Assets/Web Requests/eec266b6a3041188e438ec006d0aea27_MD5.webp]]
...
![[Assets/Web Requests/0957bc7165f2c4efc89f9d887d360ba4_MD5.webp]]
...
```shell-session
Draq4@htb[/htb]$ curl inlanefreight.com
...
```shell-session
Draq4@htb[/htb]$ curl -O inlanefreight.com/index.html
Draq4@htb[/htb]$ ls
index.html
```
...
```shell-session
Draq4@htb[/htb]$ curl -s -O inlanefreight.com/index.html
```
...
If we examine an HTTP request, we can see the effect of not enforcing secure communications between a web browser and a web application. For example, the following is the content of an HTTP login request: ![[Assets/Web Requests/4dded38627885a658727596f6c591d78_MD5.webp]]
...
In contrast, when someone intercepts and analyzes traffic from an HTTPS request, they would see something like the following: ![[Assets/Web Requests/eafb95d00a7370208d058166249ee5ec_MD5.webp]]
...
Websites that enforce HTTPS can be identified through `https://` in their URL (e.g. https://www.google.com), as well as the lock icon in the address bar of the web browser, to the left of the URL: ![[Assets/Web Requests/889c75b075dcbfbab4ff01ec030d4a7a_MD5.webp]]
...
Let's look at how HTTPS operates at a high level: ![[Assets/Web Requests/2a8da32e5c31136257fa85afdc607cdf_MD5.webp]]
...
```shell-session
Draq4@htb[/htb]$ curl https://inlanefreight.com
...
```shell-session
Draq4@htb[/htb]$ curl -k https://inlanefreight.com
...
![[Assets/Web Requests/85b29918afddb935ecb1c34deba216b4_MD5.webp]]
...
![[Assets/Web Requests/4ece425b4244a75af7cdbd8fc5483a25_MD5.webp]]
...
```shell-session
Draq4@htb[/htb]$ curl inlanefreight.com -v
...
If we click on the Network tab and refresh the page, we should be able to see the list of requests sent by the page: ![[Assets/Web Requests/f5009c49691d5825c03d1d5b448e3050_MD5.jpg]]
...
```shell-session
Draq4@htb[/htb]$ curl https://www.inlanefreight.com -A 'Mozilla/5.0'
...
Finally, let's see how we can preview the HTTP headers using the browser devtools. Just as we did in the previous section, we can go to the `Network` tab to view the different requests made by the page. We can click on any of the requests to view its details: ![[Assets/Web Requests/fea51975637bc253d04a1c42c7625938_MD5.jpg]]
...
![[Assets/Web Requests/95b345f829ec8e5437b0a9131b4b7959_MD5.jpg]]

Once we enter the credentials, we would get access to the page:

![[Assets/Web Requests/b271cc36593e3ec503a88b6b81ee85bb_MD5.jpg]]
...
```shell-session
Draq4@htb[/htb]$ curl -i http://<SERVER_IP>:<PORT>/
...
```shell-session
Draq4@htb[/htb]$ curl -u admin:admin http://<SERVER_IP>:<PORT>/
...
```shell-session
Draq4@htb[/htb]$ curl http://admin:admin@<SERVER_IP>:<PORT>/
...
```shell-session
Draq4@htb[/htb]$ curl -v http://admin:admin@<SERVER_IP>:<PORT>/
...
```shell-session
Draq4@htb[/htb]$ curl -H 'Authorization: Basic YWRtaW46YWRtaW4=' http://<SERVER_IP>:<PORT>/
...
```shell-session
Draq4@htb[/htb]$ curl 'http://<SERVER_IP>:<PORT>/search.php?search=le' -H 'Authorization: Basic YWRtaW46YWRtaW4='
...
![[Assets/Web Requests/26890a4a881cce185ed7aaa9691b71cd_MD5.jpg]]
...
```shell-session
Draq4@htb[/htb]$ curl -X POST -d 'username=admin&password=admin' http://<SERVER_IP>:<PORT>/
...
```shell-session
Draq4@htb[/htb]$ curl -X POST -d 'username=admin&password=admin' http://<SERVER_IP>:<PORT>/ -i
...
```shell-session
Draq4@htb[/htb]$ curl -b 'PHPSESSID=c1nsa6op7vtk7kdis7bcnbadf1' http://<SERVER_IP>:<PORT>/
...
```shell-session
Draq4@htb[/htb]$ curl -X POST -d '{"search":"london"}' -b 'PHPSESSID=c1nsa6op7vtk7kdis7bcnbadf1' -H 'Content-Type: application/json' http://<SERVER_IP>:<PORT>/search.php
["London (UK)"]
```
...
Finally, let's try to repeat the same above request by using `Fetch`, as we did in the previous section. We can right-click on the request and select `Copy>Copy as Fetch`, and then go to the `Console` tab and execute our code there: ![[Assets/Web Requests/d57efa725a174f071196bd28f649a411_MD5.jpg]]
...
```shell-session
Draq4@htb[/htb]$ curl http://<SERVER_IP>:<PORT>/api.php/city/london
...
```shell-session
Draq4@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/london | jq
...
```shell-session
Draq4@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/le | jq
...
```shell-session
Draq4@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/ | jq
...
```shell-session
Draq4@htb[/htb]$ curl -X POST http://<SERVER_IP>:<PORT>/api.php/city/ -d '{"city_name":"HTB_City", "country_name":"HTB"}' -H 'Content-Type: application/json'
```
...
```shell-session
Draq4@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/HTB_City | jq
...
```shell-session
Draq4@htb[/htb]$ curl -X PUT http://<SERVER_IP>:<PORT>/api.php/city/london -d '{"city_name":"New_HTB_City", "country_name":"HTB"}' -H 'Content-Type: application/json'
```
...
Draq4@htb[/htb]$ curl -s http://<SERVER_IP>:<PORT>/api.php/city/New_HTB_City | jq
...
```shell-session
Draq4@htb[/htb]$ curl -X DELETE http://<SERVER_IP>:<PORT>/api.php/city/New_HTB_City
```
